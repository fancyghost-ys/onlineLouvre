...
import { editArtPiece, getArtPiece } from '../../../store/actions/art'
...
const EditArtPiece = ({ match }) => {
    ...
    useEffect(() => {
        ...
        dispatch(getArtPiece(id)).then(data => {
            if (data.error) {
    ...
    const clickSubmit = event => {
        ...
        dispatch(editArtPiece(match.params.id, formData)).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({
    ...
    const newPostForm = () => (
        ...
        <form className="mb-3" onSubmit={clickSubmit}>
            ...
            <button class="w-100 btn btn-warning mt-4">Edit Art</button>
        </form>
    );
